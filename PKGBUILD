...
options=('!lfs')
makedepends=('libelf-lfs' xz-lfs 'zstd-lfs' openssl-lfs 'kmod-lfs')
if [ "$cc" = 'clang' ]; then makedepends+=(clang-lfs); fi


case "$LITE" in
	yes)
		source=('config-lite')
		sha256sums=('7fadd81449bf2c07711800b7b1e8505d2d9740ef6a008b371b786112c2cf9140')
		;;
	no)
		source=('config')
...
prepare(){
		...
		case $LITE in
			yes)
				diff -u ../config-lite .config > ${PKGDEST}/config-change-lite-${pkgver}.diff || true
				;;
			no)
				diff -u ../config .config > ${PKGDEST}/config-change-${pkgver}.diff || true
				;;
		esac
